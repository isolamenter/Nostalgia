...
describe('统一世界状态（store 集成）', () => {
  ...
  it('存档写入并能跨实例读回', () => {
    ...
    store.getState().setMap('demo.maps.residence')
    store.getState().applyFlag('demo.testflag', 'yes')
    store.getState().autosave()

    const store2 = createGameStore(mem)
    const ok = store2.getState().loadFromSlot('autosave')
    expect(ok).toBe(true)
    expect(store2.getState().world.flags['demo.testflag']).toBe('yes')
    expect(store2.getState().currentMap).toBe('demo.maps.residence')
  })
